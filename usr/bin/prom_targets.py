...
def generate_targets(servers, folder_path):
    ...
    for k, g in itertools.groupby(servers, key=lambda srv: srv.get('environment')):
        filename = str(k) + '.json'
        file_path_old = os.path.join(folder_path, filename)
        file_path_new = os.path.join(folder_path, filename + '.new')
        srvs = list(g)
...
def main():
    ...
    while True:
        ...
        generate_targets(servers, args.folder_path)
        time.sleep(15 * 60)
